...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!token || !email) {
    showMsg("Invalid or missing reset link. Please request a new one.", "error");
    return;
  }
  ...
  try {
    ...
    if (!users || users.length === 0) {
      showMsg("This reset link is invalid. Please request a new one.", "error");
      return;
    }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!password || !confirm) {
      showMsg("Please fill out all fields.", "error"); return;
    }
    if (password !== confirm) {
